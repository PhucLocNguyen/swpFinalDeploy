...
import loadingAnimation from "../../assets/icon/loadingAnimation.json";
...
function PaymentResponse() {
  ...
  async function verifyPayment(queryString) {
    ...
    const requirementObject = await FetchApiRequirementById(
      verifyApi.requirementId
    );
    if (!verifyApi.isFailed) {
      setStatus(true);
      if (requirementObject.status == "4") {
        PutApiRequirement(
          { ...requirementObject, status: "5" },
          "Successful",
          "Deposit Failed"
        );
      }
   ...
   await setRequirementId(verifyApi.requirementId);
    setLoading(false);
  }
